...
# Usage: ./submit_ext.sh <container_file> <r_script_file> <data_dir> <db_file> <dataset> <path_to_ext_file> <batch_size> <total_rows_in_data>
...
sbatch --array=0-${max_array_index} \
       --export=ALL,CONTAINER=$container,R_SCRIPT=$r_script,DATA_DIR=$data_dir,DB_FILE=$db_file,DATASET=$dataset,EXT_FILE=$ext_file,BATCH_SIZE=$batch_size,TOTAL_ROWS=$total_rows \
       /flashscratch/mvincent/gbrs_qtl_lods/lods_per_gene/bin/lods_per_gene_ext.sh
